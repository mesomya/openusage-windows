(function () {
  // Kiro is a VS Code-derived app; its user-data dir is per-OS. The host expands "~".
  // (TOKEN_PATH below is AWS SSO and already correct on every platform.)
  function kiroBaseDir(ctx) {
  ...
  function kiroStateDb(ctx) { return kiroBaseDir(ctx) + "/User/globalStorage/state.vscdb" }
  function kiroLogsRoot(ctx) { return kiroBaseDir(ctx) + "/logs" }
  function kiroProfilePath(ctx) { return kiroBaseDir(ctx) + "/User/globalStorage/kiro.kiroagent/profile.json" }
  ...
  function loadProfileArn(ctx, authState) {
    ...
    const parsed = readJsonFile(ctx, kiroProfilePath(ctx), "profile")
  ...
  function readStateValue(ctx, key) {
    try {
      ...
      const rows = ctx.util.tryParseJson(ctx.host.sqlite.query(kiroStateDb(ctx), sql))
  ...
  function loadLoggedState(ctx) {
    let sessions = []
    try {
      sessions = ctx.host.fs.listDir(kiroLogsRoot(ctx)).slice().sort().reverse()
    } catch {
      return null
    }
    for (let i = 0; i < sessions.length && i < 12; i += 1) {
      const sessionRoot = kiroLogsRoot(ctx) + "/" + sessions[i]
      let windows = []
      try {
